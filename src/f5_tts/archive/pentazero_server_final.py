...
# Directories for recordings, processed chunks, and generated outputs.
...
# Audio settings (all times in seconds)
min_length = 3       # Minimum chunk length
max_length = 5       # Maximum chunk length
silence_duration = 0.1  # Duration (sec) to consider as a pause
...
# Translation model configuration (using Facebook’s NLLB-200 distilled model)
...
cross_fade_duration = 0.15  # seconds for crossfade
speed = 1.0
...
# Global variable to hold the continuously crossfaded output
final_wave_data = None
final_sample_rate = 48000  # default sample rate
...
def translate_english_to_spanish(english_text: str) -> str:
    """Translates the given English text to Spanish using the NLLB model."""
...
def clip_audio(input_filepath: str, max_duration_sec: int = 6, output_filepath: str = None) -> str:
    """Clips the audio to a maximum duration and returns the clipped file path."""
...
def run_tts_inference(tts_api, ref_audio: str, ref_text: str, gen_text: str,
    ...
    """
    Runs TTS inference with the F5-TTS model.
    Returns (sample_rate, generated_wave_data).
    """
    seed = -1  # random seed
...
def crossfade_two_chunks(chunk1: np.ndarray, chunk2: np.ndarray, sr: int, duration: float = 0.15) -> np.ndarray:
    """Crossfades two audio chunks over `duration` seconds."""
...
def update_final_wave(new_chunk: np.ndarray, sr: int, crossfade_sec: float = 0.15) -> None:
    """Integrates a new generated chunk into the global crossfaded audio."""
...
def save_crossfaded_output() -> None:
    """Saves the current global crossfaded audio to disk."""
...
def process_file(filepath):
    ...
    Processes a single audio chunk:
      1. Transcribes the full chunk.
      2. Clips the audio (for a shorter TTS reference).
      3. Re-transcribes the clipped audio.
      4. Translates the full transcription.
      5. Runs TTS inference.
      6. Crossfades the generated audio into the global output.
      7. Saves and moves files appropriately.
    """
    global final_wave_data
    try:
        # 1) Transcribe the full audio chunk
        full_ref_text = transcribe_audio(filepath, language="En")
        print("Full Transcribed:", full_ref_text)

        # 2) Clip the audio to a maximum duration (e.g. 15 sec)
        clipped_filepath = clip_audio(filepath, max_duration_sec=15)
        print("Clipped audio saved to:", clipped_filepath)

        # 3) Transcribe the clipped audio for reference
        clipped_ref_text = transcribe_audio(clipped_filepath, language="En")
        print("Clipped Transcribed:", clipped_ref_text)

        # 4) Translate the full transcription from English to Spanish
        translated_text = translate_english_to_spanish(full_ref_text)
        print("Translated:", translated_text)

        # 5) Run TTS inference to generate Spanish audio
        ...
        # 6) Crossfade this new chunk into the global audio stream
        update_final_wave(wave_data, sample_rate, crossfade_sec=cross_fade_duration)

        # 7) Save the generated chunk and update processed directories
...
def process_media(filepath):
    ...
    Processes an uploaded media file (audio or video):
      ...
      - If audio and not WAV, convert to WAV.
      - Uses a fallback conversion if SoundFile fails to open the file.
      - Uses Slicer to split the audio based on silence.
      - Processes each resulting chunk.
    """
    try:
        ...
        # Try loading the audio using SoundFile.
        try:
            waveform, sr = sf.read(temp_audio_path)
        except Exception as e:
            print(f"sf.read failed with error: {e}. Attempting moviepy conversion fallback.")
        ...
        if isinstance(sliced_chunks, list):
            ...
            process_file(temp_audio_path)
    except Exception as e:
        print("Error processing media file", filepath, ":", e)

############################################
# Socket Server for Processing Requests
############################################

def handle_client(client_socket):
    """
    Receives a JSON payload (with base64‑encoded audio) from the client,
    processes the audio through the full pipeline, and streams back the final audio.
    """
    global final_wave_data, final_sample_rate
    try:
        data = b""
        ...
        if "audio" in payload and payload["audio"]:
            ...
            # Reset global crossfaded audio for a new request.
            final_wave_data = None
            process_media(input_audio_file)
            if final_wave_data is not None:
                packed_audio = struct.pack(f"{len(final_wave_data)}f", *final_wave_data)
                client_socket.sendall(packed_audio)
            else:
...
def start_server(host, port):
    """Starts the socket server to listen for incoming client connections."""
